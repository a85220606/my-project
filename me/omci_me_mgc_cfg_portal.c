...
static ca_omci_me_handler_t sg_omci_me_mgc_cfg_portal_handlers = {0};


ca_omci_me_attr_t g_omci_me_mgc_cfg_portal_attr[OMCI_ME_ATTR1] =
{
    {OMCI_ME_ATTR1, "mgc_cfg_text_tbl"},
};


ca_omci_mebase_t *ca_omci_me_mgc_cfg_portal_new(void)
{
    ca_omci_mgc_cfg_portal_t *me = (ca_omci_mgc_cfg_portal_t *)omci_malloc(sizeof(ca_omci_mgc_cfg_portal_t));
    ...
    ca_omci_base_init(&me->meBase, OMCI_ME_MGC_CFG_PORTAL_CLASS_ID, &sg_omci_me_mgc_cfg_portal_handlers);
...
ca_omci_st_e ca_omci_me_mgc_cfg_portal_get(
    ca_omci_mebase_t        *me,
    ca_omci_uint8_t          ext,
...
{
    ca_omci_mgc_cfg_portal_t *mgcCfgPortal = (ca_omci_mgc_cfg_portal_t *)me;
    ...
    OMCI_CHECK_ATTR_MASK(OMCI_MGC_CFG_PORTAL_GET_MASK_FULL);

    if (0 != OMCI_STRLEN((char *)mgcCfgPortal->attr.mgcCfgTextTbl))
    {
        ca_omci_uint32_t tblSize = OMCI_STRLEN((char *)mgcCfgPortal->attr.mgcCfgTextTbl);
        ...
        ca_omci_next_init(&mgcCfgPortal->mgcCfgTextTbl, tblSeq);
    }
    else
    {
...
ca_omci_st_e ca_omci_me_mgc_cfg_portal_get_next(
    ca_omci_mebase_t            *me,
    ca_omci_uint8_t              ext,
    ...
    ca_omci_me_getnext_result_t *result)
{
    ...
    {
        ca_omci_uint16_t tblSize = OMCI_STRLEN((char *)mgcCfgPortal->attr.mgcCfgTextTbl);
        ca_omci_uint16_t offset  = 0;
        ca_omci_uint16_t sglLen  = 0;
        ca_omci_uint16_t attLen  = 0;

        if (!ca_omci_next_valid(&mgcCfgPortal->mgcCfgTextTbl, seqNum))
        {
        ...
        OMCI_MEMCPY(attr, (mgcCfgPortal->attr.mgcCfgTextTbl + offset), attLen);
        result->attrLen = attLen;
        result->result  = OMCI_MSG_RST_SUCCESSFUL;
        ca_omci_next_success(&mgcCfgPortal->mgcCfgTextTbl);
    }
    return CA_OMCI_OK;
}


void ca_omci_me_mgc_cfg_portal_init(void)
{
    sg_omci_me_mgc_cfg_portal_handlers.getProc     = ca_omci_me_mgc_cfg_portal_get;
    sg_omci_me_mgc_cfg_portal_handlers.getNextProc = ca_omci_me_mgc_cfg_portal_get_next;
}
