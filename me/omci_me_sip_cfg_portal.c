...
static ca_omci_me_handler_t sg_omci_me_sip_cfg_portal_handlers = {0};


ca_omci_me_attr_t g_omci_me_sip_cfg_portal_attr[OMCI_ME_ATTR1] =
{
    {OMCI_ME_ATTR1, "cfg_text_tbl"},
};


ca_omci_mebase_t *ca_omci_me_sip_cfg_portal_new(void)
{
    ca_omci_sip_cfg_portal_t *me = (ca_omci_sip_cfg_portal_t *)omci_malloc(sizeof(ca_omci_sip_cfg_portal_t));
    ...
    ca_omci_base_init(&me->meBase, OMCI_ME_SIP_CFG_PORTAL_CLASS_ID, &sg_omci_me_sip_cfg_portal_handlers);
...
ca_omci_st_e ca_omci_me_sip_cfg_portal_get(
    ca_omci_mebase_t        *me,
    ca_omci_uint8_t          ext,
...
{
    ca_omci_sip_cfg_portal_t *sipCfgPortal = (ca_omci_sip_cfg_portal_t *)me;
    ...
    OMCI_CHECK_ATTR_MASK(OMCI_SIP_CFG_PORTAL_GET_MASK_FULL);

    //TODO: update value in me
    if (0 != OMCI_STRLEN((char *)sipCfgPortal->attr.cfgTextTbl))
    {
        ca_omci_uint32_t tblSize = OMCI_STRLEN((char *)sipCfgPortal->attr.cfgTextTbl);
        ...
        ca_omci_next_init(&sipCfgPortal->cfgTextTbl, tblSeq);
    }
    else
    {
...
ca_omci_st_e ca_omci_me_sip_cfg_portal_get_next(
    ca_omci_mebase_t            *me,
    ca_omci_uint8_t              ext,
    ...
    ca_omci_me_getnext_result_t *result)
{
    ...
    {
        ca_omci_uint16_t tblSize = OMCI_STRLEN((char *)sipCfgPortal->attr.cfgTextTbl);
        ca_omci_uint16_t offset  = 0;
        ca_omci_uint16_t sglLen  = 0;
        ca_omci_uint16_t attLen  = 0;

        if (!ca_omci_next_valid(&sipCfgPortal->cfgTextTbl, seqNum))
        {
        ...
        OMCI_MEMCPY(attr, (sipCfgPortal->attr.cfgTextTbl + offset), attLen);
        result->attrLen = attLen;
        result->result  = OMCI_MSG_RST_SUCCESSFUL;
        ca_omci_next_success(&sipCfgPortal->cfgTextTbl);
    }
    return CA_OMCI_OK;
}


void ca_omci_me_sip_cfg_portal_init(void)
{
    sg_omci_me_sip_cfg_portal_handlers.getProc     = ca_omci_me_sip_cfg_portal_get;
    sg_omci_me_sip_cfg_portal_handlers.getNextProc = ca_omci_me_sip_cfg_portal_get_next;
}
